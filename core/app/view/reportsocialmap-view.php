	...
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-md-12">
				...
				$users = array();
				if ((isset($_GET["q"]) && $_GET["q"] != "") || (isset($_GET["only_completed"]) && $_GET["only_completed"] != "")) {


					if (($_SESSION["user_type"] != 4 && $_SESSION["user_type"] != 5 && $_SESSION["user_type"] != 6 && $_SESSION["user_type"] != 7)) {
						$sql = "SELECT * from info_social_map WHERE user_type = 2 and info_state='" . $_SESSION["user_region"] . "' and ";
					} else {
						$sql = "SELECT * from info_social_map WHERE user_type = 2 and ";
					}
					...
					if ($_GET["only_completed"] == 1) {
						if ($_GET["q"] != "") {
							$sql .= " and ";
						}
						$sql .= " progress_percent=100 ";
					}
					...
					$url_pag = " <a href=\"?view=reportsocialmap&q=" . $_GET["q"] . "&only_completed=" . $_GET["only_completed"] . "&pag=";
					...
					if (($_SESSION["user_type"] != 4 && $_SESSION["user_type"] != 5 && $_SESSION["user_type"] != 6 && $_SESSION["user_type"] != 7)) {
						$sql = "SELECT * from info_social_map WHERE user_type = 2 and info_state='" . $_SESSION["user_region"] . "' order by id desc LIMIT " . (($compag - 1) * $CantidadMostrar) . " , " . $CantidadMostrar;
						$sql_total = "SELECT * from info_social_map WHERE user_type = 2 and info_state='" . $_SESSION["user_region"] . "'";
					} else {
					...
					$url_pag_1 = "?view=reportsocialmap&pag=";
					$url_pag = " <a href=\"?view=reportsocialmap&pag=";

					$param_csv = "info_social_map WHERE user_type = 2";
					$param_sql = "false";
				...
				<?php if ($_SESSION["user_type"] == 2 || $_SESSION["user_type"] == 3 || $_SESSION["user_type"] == 7 || $_SESSION["user_type"] == 6 || $_SESSION["user_type"] == 5 || $_SESSION["user_type"] == 4 || $_SESSION["user_type"] == 8 || $_SESSION["user_type"] == 9) { ?>
					...
					<form class="form-inline">
						...
						<div class="form-group mx-1 mb-1">
							<div class="form-check" id="only_completed">
								<label class="form-check-label">
									...
									<label class="form-check-label" for="only_completed">Solo completados</label>
							</div>
						</div>
						...
						<div class="form-group mx-1 mb-1">
							<a class="btn btn-secondary mb-2" href="core/app/view/exportxlsxmysql.php?param=<?php echo $param_csv . '&param_sql=' . $param_sql . '&filename=' . $DB_name; ?>" name="Descargar"><span class="material-symbols-outlined">download</span> XLSX</a>
						</div>
					...
					<div class="form-group">
						<div class="card-content table-responsive">
							<div class="card-body">
								...
								if (count($users) > 0) {
									// si hay usuarios
									if (isset($_GET["only_completed"])) {
										$only_completed = $_GET["only_completed"];
									} else {
										$only_completed = 0;
									}
								...
								?>
									<table id="example" class="table table-bordered table-hover">
										...
										foreach ($users as $user) {
											...
											// Get array all field name
											$array_keys = array_keys($myArray);
											$total_campos = count($array_keys);

											$campos_listos = $user->progress;
											$count_data = 0;
											$DEMO = "";
											...
											foreach ($user as $data) {
												...
												if ($data != "") {
													...
													// almacena los field que ya tienen registro
													$Update_progress[] = $array_keys[$count_data];
													// $DEMO = $data;
										...
										?>
											<tr>
												...
												<!-- <td><!?php echo count(explode($user->progress)); ?></td> -->
												...
												<td>
													<div class="progress">
														<div class="progress-bar progress-bar-striped <?php echo ($user->progress_percent == '100' ? ' bg-success ' : ' '); ?>progress-bar-animated" role="progressbar" aria-valuenow="<?php echo $campos_listos; ?>" aria-valuemin="0" aria-valuemax="<?php echo $total_campos; ?>" style="width:<?php echo $user->progress_percent; ?>%"><?php echo "     Completado: " . $campos_listos . "/" . $total_campos . " | "; ?> <?php echo $user->progress_percent . "%"; ?></div>
														<!-- <!?php echo $DEMO; ?> -->
													</div>
												</td>
											</tr>
										...
										<?php
											...
											$progress_percent = round($campos_listos * 100 / $total_campos);
											// echo $progress_percent;
											// echo implode(",",$Update_progress);
											// echo implode(",",$array_keys);

											// array_unshift($Update_progress, "progress_percent");

											// $data_progress = implode(",",$Update_progress);
											$id_progress = $user->id;
											// $updateData =  ("UPDATE info_social_map SET progress = '$data_progress'  WHERE id = '$id_progress' ");

											$updatePercent =  ("UPDATE info_social_map SET progress_percent = '$progress_percent'  WHERE id = '$id_progress' ");
											// $resultadoPercent = SocialMapData::UpdateBySQL($updatePercent);
