...
if (isset($products_type) && $products_type != null) {
    ...
    foreach ($products_type as $row) {
        ...
        array_push($pie_chart_labels, "'" . $row['name'] . "'");

    }
    $query .= " FROM products_list WHERE estate!='null' GROUP BY estate";
}

$products_by_state = $pg_con->query($query)->fetchAll(PDO::FETCH_ASSOC);
$pie_chart_labels = implode(", ", array: $pie_chart_labels);
$products_in_general = [];
$reports_total = 0;
foreach ($products_by_state as $row) {
    foreach ($row as $key => $value) {
        if (!isset($products_in_general[$key]) && $key != "estate") {
            $products_in_general[$key] = 0;
        }

        if ($value != null && $key != "estate") {
            $products_in_general[$key] += $value;
            $reports_total += $value;
        }
    }
}

$products_in_general = implode(",", $products_in_general);
...
<script>
    $(document).ready(function () {
        ...
        $(function () {
            $('#datepicker-types-products').daterangepicker({
                "showDropdowns": true,
                ranges: {
                    'Hoy': [moment(), moment()],
            ...
            $('#datepicker-types-products').on("change", function (ev, picker) {

                var start_date = ev.target.value.split(" - ")[0].replaceAll("/", "-");
                var end_date = ev.target.value.split(" - ")[1].replaceAll("/", "-");

                document.getElementById("spinner").style.display = "inline-block";
                document.getElementById("table-product-type").style.display = "none";

                $.post({

                    url: "index.php?action=ajax&function=products_by_type",
                    dataType: 'json',
                    data: {
                    ...
                    success: (response, data) => {
                        ...
                        let table = $('#table-product-type').DataTable();
                        table.clear();
                        ...
                        document.getElementById("table-product-type").style.display = "table";
                    }
                });
            });
        });
        ...
        var product_categories_colors = [];
        for (var counter1 = 0; counter1 < <?php echo $products_type->num_rows ?>; counter1++) {
            product_categories_colors.push(ColorCode());
        }
        ...
        let myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label:"Productos",
                    data: [<?php echo $products_in_general; ?>],
                    backgroundColor: product_categories_colors,
                    datalabels: {
                        color: 'white',
                        font: {
                            weight: 'bold',
        ...
        $(function () {
            ...
            $('#datepicker').on("change", function (ev, picker) {
                ...
                $.post({
                    url: "index.php?action=ajax&function=products_by_type_in_general",
                    dataType: 'json',
                    data: {
                        start_date: start_date,
                        end_date: end_date,
    ...
    <div aria-labelledby="estates-tab" class="tab-pane fade show active" id="estates" role="tabpanel">
        <div class="content">
            <div class="container">
                ...
                <h4 class="text-center mb-4 mt-2">Tipos de productos</h4>
                <div class="form-group">
                    <label for="fecha" class="control-label"><i class="fa fa-calendar"></i> Fecha de la
                        actividad</label>
                    <input type="text" name="fecha" required class="form-control" id="datepicker-types-products"
                        placeholder="Fecha">
                </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <table class="table" id="table-product-type">

                                        <thead>
                                            <tr>
                                                <th scope="col">Estado</th>
                                                <?php foreach ($products_type as $row) { ?>
                                        ...
                                        <tbody id="tableBody">
                                            <?php foreach ($products_by_state as $row) {
                                                $total = 0;
                                                ?>
                                                <tr>
    ...
    <div aria-labelledby="national-tab" class="tab-pane fade" id="national" role="tabpanel">
        <div class="offset-md-1 col-md-10">
            <div class="card text-center">
                <div class="card-body">
                    <h5 class="card-title">Tipos de productos en general</h5>
                    <div class="text-center">
                        <div class="col-md-12 ">
                            <div class="form-group">
                                <label for="fecha" class="control-label"><i class="fa fa-calendar"></i> Fecha de la
