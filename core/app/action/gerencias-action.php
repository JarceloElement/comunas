...
if ($func_get == "add") {
    ...
    $rx = GerenciasData::getRepeatedPg($_POST["document_number"]);
    if ($rx == "null") {
        $r = new GerenciasData();
...
if ($func_get == "update") {
    if (count($_POST) > 0) {
        ...
        $r = GerenciasData::getByIdPg($_POST["id"]);
        $r->id = $_POST["id"];
        $r->f_name = $_POST["f_name"];
        $r->f_lastname = $_POST["f_lastname"];
        $r->document_number = $_POST["document_number"];
...
if ($func_get == "delete") {
    ...
    $param = GerenciasData::getByIdPg($_GET["id"]);
    if ($param != "null") {
        $param->del();
        Core::redir("./index.php?view=gerencias&swal=Registro borrado");
    }
}
