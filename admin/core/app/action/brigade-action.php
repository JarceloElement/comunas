...
if ($func_get == "add_new") {
    ...
    if($_SESSION["user_type"] == 1 || $_SESSION["user_type"] == 2 || $_SESSION["user_type"] == 3 || $_SESSION["user_type"] == 8 ){
        $info_cod = $_SESSION["user_code_info"];
    }else{
        $info_cod = $_POST["info_cod"];
    }
    // Obtener el id del infocentro con su estado

    $info = InfoData::getByCode($info_cod);

    if($info == "null"){
        $array = [
            "success" => false,
            "message" => "El infocentro no existe"
        ];
...
if ($func_get == "update") {
    ...
    Core::redir("./index.php?view=brigadas&swal=Brigada actualizada con éxito");
}
...
if ($func_get == "deleteuserbrigade") {
    ...
    $param = UserBrigadesData::getById($_GET["id"]);
    
    $param->del();
        
    Core::redir("./index.php?view=brigadistas&swal=Registro borrado");
