...
<br>
<br>
<h5 for="file">AVISO: Si el mismo dato se actualiza varias veces es porque se encuentra repetido en el archivo subido.</h5>
<br>
...
// insertar
$stmt_insert = $conn->prepare("INSERT INTO gerencias (
    f_state,
    municipality,
    parish,
    info_cod,
    ...
    if ($xlsx = SimpleXLSX::parse($_FILES['dataCliente']['tmp_name'])) {
        ...
        foreach ($xlsx->rows() as $k => $fields) {
            ...
            if (!empty($param)) {
                ...
                $r = GerenciasData::getByDniPg($param);
                // print_r($r);
                // return;
