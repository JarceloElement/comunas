InfoAppJarceloInfoAppJarceloInfoAppJarcelo<?php
                                            ...
                                            function alter_db()
                                            {
                                                ...
                                                $organizaciones = $conn->prepare("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'organizaciones' ");
                                                $organizaciones->execute();
                                                $table_name = $organizaciones->fetch();

                                                // Si no existe la tabla, se crea desde el archivo
                                                if (!isset($table_name["table_name"])) {
                                                    $query = '';
                                                    $sqlFile = file('./../migrate/organizaciones-Pg.sql');
                                                ...
                                                $formaciones = $conn->prepare("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'formaciones' ");
                                                $formaciones->execute();
                                                $table_name = $formaciones->fetch();

                                                // Si no existe la tabla, se crea desde el archivo
                                                if (!isset($table_name["table_name"])) {
                                                    $query = '';
                                                    $sqlFile = file('./../migrate/formaciones-Pg.sql');
                                                ...
                                                $certificados = $conn->prepare("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'certificados' ");
                                                $certificados->execute();
                                                $table_name = $certificados->fetch();

                                                // Si no existe la tabla, se crea desde el archivo
                                                if (!isset($table_name["table_name"])) {
                                                    $query = '';
                                                    $sqlFile = file('./../migrate/certificados-Pg.sql');
                                                ...
                                                // ALTER TABLE facilitators ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;
                                                ...
                                                // CREATE UNIQUE INDEX IF NOT EXISTS idx_info_code ON public.infocentros(cod)
                                                ExecutorPg::doit('CREATE INDEX IF NOT EXISTS idx_final_users_dni ON public.final_users(user_dni);');
