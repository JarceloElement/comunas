	...
	<div class="row">
		<div class="col-md-12">
			<div class="row justify-content-center container">
				...
				<div class="col-md-4">
					<div class="row justify-content-center container">
						<form action="index.php?view=import_xlsx_gerenciasPg" method="POST" enctype="multipart/form-data" />
			...
			<div class="col-md-12">
				<div class="card">
					...
					<div class="card-header card-header-primary">
						<h4 class="card-title">Filtrar gerentes</h4>
					...
					<div class="form-group">
						...
						<div class="card-body">
							<form class="form-horizontal" role="form">
								<input type="hidden" name="view" value="gerencias">
...
$sql_rrss = "SELECT 
gerencias.f_state, 
gerencias.status_nom, 
gerencias.personal_type, 
gerencias.f_name, 
gerencias.f_lastname, 
gerencias.document_number, 
gerencias.phone_number, 
gerencias.gender, 
gerencias.email, 
final_users.red_x, 
...
final_users.red_pinterest from gerencias INNER JOIN final_users ON (final_users.user_dni = gerencias.document_number) where";
...
if ((isset($_GET["q"]) && isset($_GET["estado"])) && ($_GET["q"] != "" || $_GET["estado"] != "")) {
	...
	if ($_GET["q"] != "") {
		...
		$sql_rrss .= " (gerencias.email = '$_GET[q]' or gerencias.status_nom like '%$_GET[q]%' or gerencias.f_name like '%$_GET[q]%' or gerencias.document_number like '%$_GET[q]%' or gerencias.f_lastname like '%$_GET[q]%')";
	}
	...
	if ($_GET["estado"] != "") {
		if ($_SESSION["user_type"] == 8 || $_SESSION["user_type"] == 3) {
			...
			$sql_rrss .= " gerencias.f_state ='" . $user_region . "'";
			...
			$sql .= " f_state ='" . $estate . "'";
			$sql_rrss .= " gerencias.f_state ='" . $estate . "'";
		} else {
			if ($_GET["q"] != "") {
				$sql .= " and ";
				$sql_rrss .= " and ";
			...
			$sql_rrss .= " gerencias.f_state ='" . $_SESSION["user_region"] . "'";
		}
	}
	...
	if ($_GET["municipio"] != "") {
		...
		$sql_rrss .= " gerencias.municipality ='" . $municipality . "'";
	}
	...
	$users = GerenciasData::getAllPg($sql);
	$TotalReg = $users[1];
	...
	if ($_SESSION["user_type"] == 8 || $_SESSION["user_type"] == 3) {
		$sql = "SELECT * from gerencias WHERE f_state ='" . $user_region . "' order by id asc";
		$users = GerenciasData::getAllPg($sql);
		$TotalReg = $users[1];
		$sql_dw = "SELECT * from gerencias WHERE f_state ='" . $user_region . "' order by id asc";
		$sql_rrss = $sql_rrss . " gerencias.f_state ='" . $user_region . "' and (final_users.red_x!='' or final_users.red_facebook!='' or final_users.red_instagram!='' or final_users.red_linkedin!='' or final_users.red_youtube!='' or final_users.red_tiktok!='' or final_users.red_whatsapp!='' or final_users.red_telegram!='' or final_users.red_snapchat!='')";
		$sql .= " LIMIT " . $CantidadMostrar . " OFFSET " . (($compag - 1) * $CantidadMostrar);
	} else {
		$sql = "SELECT * from gerencias order by id asc";
		$sql_dw = "SELECT * from gerencias order by id asc";
		...
		$row_table = $conn->prepare("SELECT reltuples::integer FROM pg_class WHERE relname = 'gerencias'");
		$row_table->execute();
...
<div class="col-md-12">
	...
	<?php if ($_SESSION["user_type"] != 10 && $_SESSION["user_type"] != 3) { ?>
		<a href="./index.php?view=newgerencia" class="btn btn-info">Agregar gerente</a>
	<?php } ?>
</div>
								...
								<td style="width:180px;">
									<?php if ($_SESSION["user_type"] != 10) { ?>
										<?php if ($_SESSION["user_type"] == 6 || $_SESSION["user_type"] == 7) { ?>
											...
											<a href="index.php?view=editgerente&id=<?php echo $user["id"]; ?>" class="btn btn-warning btn-sm"><i><svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24">
											...
											<?php $URL = "./?action=gerencias&function=delete&id=" . $user["id"]; ?>
										...
										<?php } elseif (($_SESSION["user_type"] == 4 || $_SESSION["user_type"] == 5 || $_SESSION["user_type"] == 6 || $_SESSION["user_type"] == 7 || $_SESSION["user_type"] == 8 || $_SESSION["user_type"] == 9) && ($_SESSION["user_region"] == $user["f_state"])) { ?>
											...
											<a href="index.php?view=editcoordinator&id=<?php echo $user["id"]; ?>" class="btn btn-warning btn-sm"><i><svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24">
														<path fill="currentColor" d="M3 21v-4.25L16.2 3.575q.3-.275.663-.425t.762-.15t.775.15t.65.45L20.425 5q.3.275.438.65T21 6.4q0 .4-.137.763t-.438.662L7.25 21zM17.6 7.8L19 6.4L17.6 5l-1.4 1.4z" />
													</svg></i></a>
											<!-- <a href="./?action=gerencias&function=delete&id=<!?php echo $user->id;?>" class="btn btn-danger btn-sm"><i class="material-icons">close</i></a> -->
