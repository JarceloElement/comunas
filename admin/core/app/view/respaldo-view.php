...
<div class="card text-center">
	...
	<div class="card-body">
		...
		<a href="./../index.php?view=userform_new" class="btn btn-primary">Nuevo usuario</a>
...
$users= array();
if( (isset($_GET["q"]) || isset($_GET["start_at"]) || isset($_GET["finish_at"]) || isset($_GET["uid"]) || isset($_GET["user_estado"]) ) &&  ($q!="" || $_GET["start_at"]!="" || $_GET["finish_at"]!="" || $uid_q!="" || $_GET["user_estado"]!="" ) ) {
	...
	if($_GET["start_at"]!="" and $_GET["finish_at"]!=""){
		if($_GET["q"] != "" or $_GET["user_estado"] != "" or $_GET["uid"] != ""){
			$sql .= ' and ';
		}
	...
	$users = ServicesUsersData::getBySQL($sql." order by user_fecha_reg desc LIMIT ".(($compag-1)*$CantidadMostrar)." , ".$CantidadMostrar);

	// Asigna url de paginacion
	$url_pag = "<a href=\"?view=services&q=".$q."&user_estado=".$_GET["user_estado"]."&start_at=".$_GET["start_at"]."&finish_at=".$_GET["finish_at"]."&pag=";
	...
	// $total = ServicesUsersData::getAll();
	$sql_t = "SELECT  * from services_users where user_info_cod != '' ";
	$sql_total = ServicesUsersData::getBySQL($sql_t);
	$TotalReg = count($sql_total);

	$sql = "SELECT  * from services_users where user_info_cod != '' order by user_fecha_reg desc LIMIT ".(($compag-1)*$CantidadMostrar)." , ".$CantidadMostrar;
	$users = ServicesUsersData::getBySQL($sql);

	$url_pag = "<a href=\"?view=services&q=".$q."&user_estado=".$estado_q."&start_at=&finish_at&pag=";
...
<div class="col-md-12">
	...
	<div class="col-md-12">
		<div class="input-group">
			...
			<a target="_blank" class="btn btn-success" href="../../../core/app/view/exportxlsx.php?param=<?php echo $param_csv.'&param_sql=true&filename='.$DB_name; ?>" name="Descargar"><i class="fa fa-file-excel-o"></i> XLSX</a>
				...
				<table  class="table table-bordered table-hover">
						...
						foreach($users as $user){ ?>
							<tr>
								...
								<td><?php echo $user->user_nombres; echo " ".$user->user_apellidos; ?></td>
								...
								<td>
									<?php if ($_SESSION["user_type"] != 10){ ?>
										<?php if ($_SESSION["user_type"] == 5 || $_SESSION["user_type"] == 6 || $_SESSION["user_type"] == 7){ ?>
											<a href="index.php?action=services_users&function=delete&id=<?php echo $user->id;?>&uid=<?php echo $uid_q; ?>&q=<?php echo $q; ?>&user_estado=<?php echo $user->user_estado; ?>&start_at=<?php echo $start_at_q;?>&finish_at=<?php echo $finish_at_q;?>&pag=<?php echo $pag;?>" class="btn btn-danger btn-sm"><i class="material-icons">close</i></a>
