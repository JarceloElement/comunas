		...
		<div class="card">
			...
			<div class="card-content table-responsive">
				...
				if( ( isset($_GET["q"]) && isset($_GET["start_at"]) && isset($_GET["finish_at"]) ) && ( $_GET["q"]!="" || $_GET["start_at"]!="" || $_GET["finish_at"]!="" ) ) {
					...
					$sql = "select * from participants_list where ";

					if($_GET["q"]!=""){
						$sql .= " (code_info like '%$_GET[q]%' or estate like '%$_GET[q]%' or name like '%$_GET[q]%' or document_id like '%$_GET[q]%' or age like '%$_GET[q]%' or gender like '%$_GET[q]%' or name_activity like '%$_GET[q]%') ";
					}
					...
					if($_GET["start_at"]!="" and $_GET["finish_at"]!=""){
						...
						$sql .= " ( date_activity >= '".$_GET["start_at"]."'"." and date_activity <= '".$_GET["finish_at"]."'"." ) ";
					}
					...
					if($_GET["start_at"]!="" and $_GET["finish_at"]==""){
						...
						$sql .= " ( date_activity >= '".$_GET["start_at"]."'"." ) ";
					}
					...
					$param = $sql;
					$total = ParticipantsData::getBySQL($param);
					$TotalReg = count($total[0]);

					$users = ParticipantsData::getBySQL($sql." order by date_activity desc LIMIT ".(($compag-1)*$CantidadMostrar)." , ".$CantidadMostrar);

					// Asigna url de paginacion
					// $url_pag = "<a href=\"?view=participants&q=".$_GET["q"]."&start_at=&finish_at=&pag=";
					$url_pag = "<a href=\"?view=participants&q=".$_GET["q"]."&start_at=".$_GET["start_at"]."&finish_at=".$_GET["finish_at"]."&pag=";
					...
					$total = ParticipantsData::getAll();
					$TotalReg = count($total)[0];
					$sql = "select * from participants_list order by date_activity desc LIMIT ".(($compag-1)*$CantidadMostrar)." , ".$CantidadMostrar;
					$users = ParticipantsData::getBySQL($sql);

					$url_pag = "<a href=\"?view=participants&q=".$_GET["q"]."&start_at=".$_GET["start_at"]."&finish_at=".$_GET["finish_at"]."&pag=";

					$param_csv = "participants_list order by date_activity desc";
					$param_sql = "false";
				...
				<a href="./pdf/jspdf_part.php?param_pdf=<?php echo $param_csv.'&param_sql='.$param_sql.'&DB_name='.$DB_name; ?>" name="Descargar" class=" btn btn-danger "><i class="fa fa-file-pdf-o"></i> </a>
			...
			<div class="card-content table-responsive">
				...
				<table id="table" class="table table-bordered table-hover">
					...
					foreach($users as $user){
						...
						<td><?php echo date("d/m/Y", strtotime($user->date_activity)); ?></td>
						...
						<?php $_SESSION["location"] = "?view=participants&location=participants&q=".$user->estate."&start_at=&finish_at=&id=".$user->id;?>

						<?php if ($_SESSION["user_region"] == $user->estate){ ?>

							<a href="index.php?view=editparticipants&location=participants&id=<?php echo $user->id;?>&pag=<?php echo $_GET["pag"];?>" class="btn btn-warning btn-xs">Editar</a>
						...
						<?php }elseif ( $_SESSION["user_type"] == 4 || $_SESSION["user_type"] == 5 || $_SESSION["user_type"] == 6 || $_SESSION["user_type"] == 7 || $_SESSION["user_type"] == 8 || $_SESSION["user_type"] == 9 ) { ?>
							...
							<a href="index.php?view=editparticipants&location=participants&id=<?php echo $user->id;?>&id_activity=<?php echo $user->id_activity;?>&code_info=<?php echo $user->code_info;?>&pag=<?php echo $_GET["pag"];?>" class="btn btn-warning btn-xs">Editar</a>
