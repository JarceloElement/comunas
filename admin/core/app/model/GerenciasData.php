...
class GerenciasData {
	...
	public function update(){
		...
		if ($this->f_state >= 1 and $this->f_state <= 100){
			$estado_n = EstadoData::getById($this->f_state);
			foreach($estado_n as $p):
				$this->estado_name = $p['estado'];
			endforeach;
		...
		$sql = "update gerencias set 
		name=\"$this->f_name\", 
		lastname=\"$this->f_lastname\", 
	...
	public static function getByIdPg($id)
	{
		...
		$array = ModelPg::one($query[0][0], new GerenciasData());
		if ($array->id == "") {
			return "null";
		} else {
			return $array;
