...
// partiendo de que ya tenemos creado un objecto UserData previamente utilizamos el contexto
	...
	public static function getByDniPg($dni){
		$sql = "select * from users where user_dni='$dni' and user_dni!='No cedulado' ";
		$query = ExecutorPg::doit($sql);
		$array = ModelPg::one($query[0][0], new UserData());
		if ($array->id == "") {
			return "null";
		} else {
			return $array;
	...
	public static function getRepeatedUser($user){
	...
	public static function getRepeatedUserEdit($user,$id){
		$sql = "select * from ".self::$tablename." where username=\"$user\" AND id!=\"$id\" ";
		$query = Executor::doit($sql);
		return Model::one($query[0],new UserData());
	}

	public static function getRepeatedEmailEdit($email,$id){
		$sql = "select * from ".self::$tablename." where email=\"$email\" AND id!=\"$id\" ";
		$query = Executor::doit($sql);
		return Model::one($query[0],new UserData());
	}

	public static function getRepeatedDniEdit($dni,$id){
		$sql = "select * from ".self::$tablename." where user_dni=\"$dni\" AND id!=\"$id\" ";
		$query = Executor::doit($sql);
		return Model::one($query[0],new UserData());
	}
