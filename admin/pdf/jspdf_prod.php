...
function generate() {
	...
	var columns = ["Nº","Actividad", "Fecha", "Región", "Cod-Infc","Acción realizada","Formato","Detalles del formato","Cantidad creados","Cantidad publicados"];
	var data = [
		<?php foreach($res as $row):
			$n++?>
		['<?php echo $n; ?>','<?php echo str_replace("\r\n", ' ',$row["activity_title"]); ?>','<?php echo date("d/m/Y",strtotime(explode("/",$row["date"])[0])); ?>','<?php echo $row["estate"]; ?>','<?php echo $row["code_info"]; ?>','<?php echo $row["action_performed"]; ?>','<?php echo $row["format"]; ?>','<?php echo $row["format_detail"]; ?>','<?php echo $row["quantity_created"]; ?>','<?php echo $row["quantity_published"]; ?>'],
		<?php endforeach; ?>
	];
