...
function generate() {
	...
	var columns = ["Nº","Actividad", "Fecha", "Región", "Cod-Infc","Nombre","Documento ID","Edad","Género","Teléfono"];
	var data = [
		<?php foreach($res as $row):
			$n++?>
		['<?php echo $n; ?>','<?php echo str_replace("\r\n", ' ',$row["name_activity"]); ?>','<?php echo date("d/m/Y",strtotime(explode("/",$row["date_activity"])[0])); ?>','<?php echo $row["estate"]; ?>','<?php echo $row["code_info"]; ?>','<?php echo $row["name"]; ?>','<?php echo $row["document_id"]; ?>','<?php echo $row["age"]; ?>','<?php echo $row["gender"]; ?>','<?php echo $row["phone"]; ?>'],
		<?php endforeach; ?>
	];
